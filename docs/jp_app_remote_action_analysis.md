...
| CANCELLED の扱い | 失敗 | 失敗 | ✅ |
| **`targetTemperature`** | **送らない** | **21 を送る** | ❌ |
| **`X-Vehicle-Gateway`** | **gateway 名 (AVN/NGDC/…)** | **VIN** | ❌ |
| **ポーリング打ち切り** | **200〜400 秒** | **60 秒** | ❌ |
| シート/デフロスタのキー | 装備があれば付く | 付けない | 車の features 次第（未確認） |
| エンジン停止 | `/actions/engine-start` + `action:"stop"` | 未実装 | ❌（分岐条件未特定のため未実装） |
| 20分ボタンの出し分け | `engineDoubleStart` / `hvacOnRes20mins` の可用性 | 無条件に表示 | ❌（features のキー対応が未特定） |
...
### 3.3 20分ボタンの可用性判定のキー
- アプリ内では `RemoteActionFeature.engineDoubleStart` / `hvacOnRes20mins`。features JSON のどのキー・どの階層に対応するかは未特定。
