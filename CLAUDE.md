...
| `custom_components/nissan_connect/kamereon/kamereon.py` | EU/JP 共通の API クライアント (`KamereonSession` / `Vehicle`) |
...
- EU 向けの既存挙動を壊さない。JP 固有の分岐は `session.region == 'JP'` / Feature の有無で判定し、共通コードには最小限の変更にとどめる。
