...
settings_map = {
    'nissan': {
        'JP': {
            # --- NissanConnect アプリ (BFF) ---
            'app_id': 'jp.co.nissan.nissanconnect.ncx',
            # --- KAuth (ForgeRock) 直接ログイン用 ---
...
class KamereonSession:
    ...
    def login(self, username=None, password=None):
        ...
        # 1) 従来どおり BFF のログインAPIを試す（日産側が正常ならこれが最短）
        problems = []
        try:
            token = self._login_via_bff(username, password)
            ...
            # 2) KAuth (ForgeRock) に直接ログインする
            try:
                token = self._login_via_kauth(username, password)
    ...
    def _login_via_bff(self, username, password):
        """旧 NissanConnect アプリ (NCX) と同じ、BFF 経由のログイン。"""
        ...
        if 'errors' in auth_result:
            error = auth_result['errors'][0]
            raise RuntimeError(error.get('detail', str(error)))
        return auth_result["data"]["attributes"]["access_token"]

    def _login_via_kauth(self, username, password):
        """KAuth (ForgeRock) に直接ログインしてアクセストークンを得る。"""
        if not self.settings.get('auth_base_url'):
            raise RuntimeError('KAuth login is not configured for this region')

        base_url = self.settings['auth_base_url']
        realm = self.settings['realm']
        session = requests.session()

        # --- 1段目: 認証チャレンジ (callbacks) を取得 ---
        ...
        # --- 3段目: authorization code を受け取る ---
        authorize_url = '{}oauth2{}/authorize'.format(base_url, oauth_realm)
        ...
        if client_secret:
            attempts.append(
                ('client_secret_post', dict(payload, client_secret=client_secret), None))
            attempts.append(
                ('client_secret_basic', payload, (self.settings['client_id'], client_secret)))
...
class Vehicle:
    ...
    def __init__(self, data, user_id):
        ...
        self.color = data.get('color')['nameG2B']
        ...
        self.model_name = data.get('model')['displayName']
        self.model_code = data.get('model')['code']
        self.model_year = data.get('model')['year']
        self.nickname = data.get('model')['name']
        self.phase = None #data.get('phase')
        self.picture_url = data.get('bfpImageUrl')
        self.privacy_mode = None #data.get('privacyMode')
    ...
    def initiate_srp(self):
        ...
        body = resp.json()
        _LOGGER.debug(f"initiate_srp: {body['data']}")
        if 'errors' in body:
            raise ValueError(body['errors'])
        return body
    ...
    def validate_srp(self):
        ...
        body = resp.json()
        _LOGGER.debug(f"validate_srp: {body['data']}")
        if 'errors' in body:
            raise ValueError(body['errors'])
        return body
