...
- Sensors
  ...
  - Fuel Autonomy
  - Fuel Level
  - Remote Engine Status (raw value) and Remote engine state
...
- Binary Sensors
  - Doors Locked
  - Doors Open
  - Warning lights: ABS, Airbag, Brake, Malfunction Indicator Lamp (engine), Oil Pressure
- Device Tracker
  - Location
- Buttons
  - Start Engine (no temperature setting, just a start trigger like the app's own button)
  - Stop Engine (tries the GraphQL route first and falls back to the legacy route on failure)
  - Lock Doors (the app itself has no remote unlock action)
  - Update Data
